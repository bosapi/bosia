# Bosia Demo — Cloudflare Workers

A mirror of [`apps/demo`](../demo/README.md) built for Cloudflare Workers (`target: "workers"` in `bosia.config.ts`). The Bun-only uploads routes are left out, and `/platform` shows a binding from `wrangler.jsonc` reaching a loader.

The demo app for the [Bosia](../../packages/bosia/README.md) framework. It demonstrates file-based routing, server loaders, API endpoints, middleware hooks, and Tailwind design tokens.

## Running

```bash
bun run dev     # http://localhost:9000 (Bun, like any Bosia app)
bun run build   # production build → dist/worker/index.js
bun run start   # run the worker locally in wrangler dev
...
| `/platform`   | `(public)/platform/+page.svelte`      | Reads `platform.env` (wrangler vars)  |
