	...
	<g transform="translate({MARGIN.left},{MARGIN.top})">
		{#if prepared}
			{#each prepared.yTicks as tick}
				...
				<text
					...
					font-size="10"
					fill="currentColor"
					fill-opacity="0.45">{formatNumber(tick)}</text
				>
			{/each}
			...
			{#each prepared.xLabels as xl}
				<text
					...
					font-size="10"
					fill="currentColor"
					fill-opacity="0.45">{xl.label}</text
				>
			{/each}
		...
		{:else}
			<text
				...
				font-size="12"
				fill="currentColor"
				fill-opacity="0.35">No data</text
			>
		{/if}
	</g>
</svg>
