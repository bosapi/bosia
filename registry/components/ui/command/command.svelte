<script lang="ts">
	...
	const visibleItems = $derived(items.filter((i) => filter(i.value, query, i.keywords)));

	const highlightedValue = $derived.by(() => {
		if (!visibleItems.length) return undefined;
		if (userHighlight && visibleItems.find((i) => i.value === userHighlight)) {
	...
	function moveHighlight(dir: 1 | -1 | "first" | "last") {
		...
		const nextIdx = idx === -1 ? 0 : (idx + dir + visibleItems.length) % visibleItems.length;
