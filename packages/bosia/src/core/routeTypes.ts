...
function paramsForDir(dir: string): string[] {
    if (dir === ".") return [];
    const segs = dir.split("/").filter(Boolean).filter(s => !/^\(.*\)$/.test(s));
    const out: string[] = [];
    for (const seg of segs) {
        const catchAll = seg.match(/^\[\.\.\.(\w+)\]$/);
        if (catchAll) { out.push(catchAll[1]!); continue; }
...
export function generateRouteTypes(manifest: RouteManifest): void {
    ...
    for (const [dir, info] of dirs) {
        ...
        if (info.pageServer) {
            ...
            lines.push(`export type PageMetadataLoad = (event: _MetadataEvent) => Metadata | Promise<Metadata>;`);
            lines.push(`export type Action = (event: _RequestEvent) => any;`);
            lines.push(`export type PageData = Awaited<ReturnType<typeof _pageLoad>> & { params: Params };`);
        } else {
            lines.push(``);
            lines.push(`export type PageMetadataLoad = (event: _MetadataEvent) => Metadata | Promise<Metadata>;`);
            lines.push(`export type Action = (event: _RequestEvent) => any;`);
        ...
        if (info.layoutServer) {
            ...
            lines.push(`export type LayoutData = Awaited<ReturnType<typeof _layoutLoad>> & { params: Params };`);
