...
- [x] 🟠 Client: emit `parentSnapshots: Record<depth, Record<string, any>>` for skipped layers. Done in `core/client/App.svelte` (nav) + `core/client/prefetch.ts` (prefetch) via a shared `buildParentSnapshots` helper — POST with `{ parentSnapshots }` only when non-empty, else keep the cacheable GET. (router logic lives in App.svelte, not router.svelte.ts.)
...
- [x] 🟠 `packages/bosia/src/core/renderer.ts` — `loadRouteData` accepts `parentSnapshots?: Record<number, Record<string, any>>`; in the skip branch, `parentData = { ...parentData, ...(parentSnapshots?.[ls.depth] ?? {}) }` so downstream loaders see the same data they would on SSR. Trust boundary note: client-supplied parent data is just a perf hint, never authoritative — code that relies on auth/role/farmId for authorization must still read from `event.locals` populated by `hooks.server.ts`.
- [x] ⚪ `docs/content/skills/bosia-routing/SKILL.md` — added rule under R3: don't use `parent()` for scope identifiers (farmId/orgId/userId), read from `event.locals`; `parent()` is fine for view-layer data, `locals` is the source of truth for cross-loader scope + authz.
- [ ] ⚪ Regression test under `apps/demo` (or new `apps/parent-data-nav`): a layout server returning `{ orgId }`, a child `+page.server.ts` asserting `(await parent()).orgId === "..."`. Verify SSR pass AND a client-nav JSON fetch pass. (Not yet added — needs a client-nav harness; tracked as follow-up.)
