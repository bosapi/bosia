...
- **Bundle size.** The free tier allows 3 MB gzipped. The demo app is about 185 KB. Plugins imported by `bosia.config.ts` are bundled even when they only act in dev, but the Svelte compiler they use is left out.
