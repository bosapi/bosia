...
| `BOSIA_REUSE_PORT`        | `0`                                        | Set to `1` to allow several server processes to share one port (`SO_REUSEPORT`), for deliberate multi-process clustering. Off by default: otherwise a second `bosia start` silently joins the port and the kernel splits traffic between two different builds. With it off, the second server refuses to start and exits non-zero.                                                                                                                                                    |
| `BASE_PATH`               | `""` (origin root)                         | Mount the whole app under a URL prefix, e.g. `/sso`. Bosia strips it before routing and puts it back on redirects, framework asset URLs, cookies and the client router, so routes and `event.url.pathname` stay prefix-free. Requests outside the base 404. Must be set for `bosia build` as well as `bosia start`. URLs your own components build need the `base` export. See [Deployment › Mounting Under a Sub-Path](/reference/deployment/#mounting-under-a-sub-path).            |
...
| `CSRF_EXEMPT_PATHS`       | —                                          | Comma-separated request paths exempt from the CSRF origin check, for server-to-server webhooks that send no `Origin`/`Referer`. Matched exact or on a path boundary (`/webhook` covers `/webhook/…` but not `/webhooky`). Exempt routes bypass CSRF, so they **must** authenticate the caller themselves (verify a webhook token/signature). See [Security › Exempting webhook paths](/guides/security/#exempting-webhook-paths-csrf_exempt_paths).                                   |
