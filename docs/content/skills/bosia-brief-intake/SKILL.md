...
4. `fs_write("BRIEF.md", ...)` with the consolidated answers.
5. `read_skill({ name: "bosia-brief-review" })` and walk its checklist.
6. Set `## Status: complete` in BRIEF.md.
7. Only now: greet user with a recap and the suggested first build step.
...
Then fill the remaining fields with sensible defaults (see each group skill) and confirm in one block before writing BRIEF.md.
...
- [ ] Identity, Voice, Visual, Platform sections all populated (no `TBD`).
...
- `bosia-brief-identity`, `bosia-brief-voice`, `bosia-brief-visual`, `bosia-brief-platform`, `bosia-brief-review`.
