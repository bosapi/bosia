---
name: bosia-navbars
description: Catalog of 18 navbar blocks (standard, themed, app/interactive). Install with `bosia add block navbars/<name>`; self-contained `<header>` sections that restyle across all 19 themes via semantic tokens.
triggers:
  - navbar
  - nav bar
  - header
...
Bosia ships **18 navbar blocks** ported from the Navbar Stock design system. Each is a
...
**Themes** (6) — `dark` (inverted bar), `glass` (translucent blur), `brutalist` (primary bar,
thick borders, mono caps, hard shadow), `pill` (floating capsule), `gradient` (dark gradient +
primary glow + beta pill), `lime` (full-colour primary brand bar).
